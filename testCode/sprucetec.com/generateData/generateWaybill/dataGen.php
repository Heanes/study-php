...
/*
create waybill
cli php.ini enable pdo extension
php dataGen.php [YMD|default today] [count per day|default 10] [prefix of waybill_no|default w_] [days|default 1]
example:

php dataGen.php 2016-03-11 1 waybill_ 1
*/

//$start_ymd = isset($argv[1]) ? $argv[1] : date("Y-m-d");
//$day_count = isset($argv[2]) ? (int)$argv[2] : 1;
//$prefix = isset($argv[3]) ? $argv[3] : "w_";
//$days = isset($argv[4]) ? (int)$argv[4] : 1;
//
//$conf = [
//	"day_count"				=> $day_count,
//	"city_id" 				=> 1,
...
//	"seq" 					=> 0,
//	"prefix"	 			=> $prefix,
//	"delivery_time" 		=> strtotime($start_ymd), // today
//];
//
//$tmc = _::app()->db("tmc");
//$succ = 0;
//for ($i=1; $i <= $days; $i++) {
//	$succ += createOneDayWaybill($conf, $tmc);
//	_::log($succ);
//	$start_ymd += 3600 * 24;
//}

$conf = $_POST["conf"];
$start_ymd = $conf["delivery_time"];
$conf["delivery_time"] = strtotime($conf["delivery_time"]);
